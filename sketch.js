...
function draw() {
     ...
     if(gameState === PLAY){         
          ...
          if ((touches.length > 0 ||keyDown("space")) && dog.y >= height/2) {
               jumpSound.play();
     ...
     textSize(width/35);
     fill("black");
     // Concatené la variable de Score T con el + así: 
...
function spawnObstacles(){
     if(frameCount % 80 === 0){
          // crear una variable y generar numero random para añadir a la posición en X y Y 
          var posicionRandomX = Math.round(random(width/2, width));
          var posicionRandomY = Math.round(random(height/2, height-200))
          end = createSprite(posicionRandomX,posicionRandomY,100,30);
...
function spawnAwards(){
     if(frameCount % 225 === 0){
          ...
          var posicionRandomX = Math.round(random(width/2, width));
          var posicionRandomY = Math.round(random(height/2, height-200));
          win = createSprite(posicionRandomX,posicionRandomY,100,30);
